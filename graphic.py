...
class Model:
    ...
    def build_model(self, dataset, nb_classes = 5):
        ...
        #输出模型概况
        self.model.summary()  
        plot_model(self.model, to_file='example.png', show_shapes=True)
        net = mpimg.imread('example.png')    
        net.shape
        plt.imshow(net)    
        plt.axis('off')
        plt.show()
    #训练模型
    def train(self, dataset, batch_size = 20, nb_epoch = 16, data_augmentation = True):        
